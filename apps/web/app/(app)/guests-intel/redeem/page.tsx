...
import { canAccessRedeem, canEditRedeem } from "@/lib/guests-intel/permissions";
import { getGuestsIntelRedeemPage } from "@/lib/guests-intel/page-context";

export default async function GuestsIntelRedeemPage() {
  const { venue, permissions, supabase } = await getGuestsIntelRedeemPage();

  if (!canAccessRedeem(permissions, venue.id)) {
  ...
  try {
    ...
    return (
      <div className="mx-auto w-full space-y-6 lg:w-2/3">
        <div>
          <ModulePageTitle>Redeem</ModulePageTitle>
        ...
        <VouchersPanel
          ...
          canEdit={
            canEditVouchers(permissions, venue.id) ||
            canEditRedeem(permissions, venue.id)
          }
          defaultWorkspace="redeem"
        />
      </div>
    );
  } catch (error) {
    ...
    console.error("[guests-intel/redeem]", error);
