...
export default async function HrBenefitsGratuityPage() {
  ...
  if (error && !migrationRequired) {
    console.error("[hr/benefits/gratuity] list runs:", error.message);
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-serif text-lg text-[#3D421F]">Gratuity</h2>
        <p className="text-sm text-black/55">
          Monthly tip collection, tip-out, general pool, and departmental
          distribution. Values pull from waiter sales (cash / CC tips, revenue,
          ASPH) and staff attendance when a run is calculated.
        </p>
      </div>
