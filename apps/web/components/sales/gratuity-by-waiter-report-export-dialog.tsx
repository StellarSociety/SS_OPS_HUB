...
import { exportGratuityByWaiterReport } from "@/lib/sales/gratuity-by-waiter-report-export";
import {
...
type GratuityByWaiterReportExportDialogProps = {
  open: boolean;
  venueName: string;
  ...
  /** Fixed waiter filter from the preview page. */
  waiterId?: string;
...
const FORMAT_OPTIONS: Array<{
...
}> = [
  {
    value: "pdf",
    label: "PDF",
    description: "Printable table by day and waiter",
    icon: FileText,
  },
  {
    value: "excel",
...
export function GratuityByWaiterReportExportDialog({
  open,
  venueName,
  venueLogoUrl,
  userDisplayName,
  report: fixedReport,
  waiterRecords,
  waiterId: fixedWaiterId,
  allowMonthSelect = false,
  onClose,
}: GratuityByWaiterReportExportDialogProps) {
  ...
  const [waiterId, setWaiterId] = useState(
    () => fixedWaiterId ?? GRATUITY_BY_WAITER_ALL,
  );
  ...
  const showWaiterSelect = showMonthSelect;

  const waiterOptions = useMemo(
    () => listGratuityByWaiterOptions(waiterRecords ?? [], month),
    [waiterRecords, month],
  );
  ...
  useEffect(() => {
    ...
    setWaiterId(fixedWaiterId ?? GRATUITY_BY_WAITER_ALL);
  }, [open, fixedReport?.monthKey, fixedWaiterId, availableMonths]);

  useEffect(() => {
    if (!open || !showWaiterSelect) return;
  ...
  }, [open, showWaiterSelect, waiterId, waiterOptions]);
  ...
  const report = useMemo(() => {
    ...
    return buildGratuityByWaiterReportMonth(waiterRecords ?? [], month, {
      waiterId,
    });
  }, [showMonthSelect, fixedReport, waiterRecords, month, waiterId]);

  async function handleExport() {
    setExporting(true);
    setError(null);
    try {
      await exportGratuityByWaiterReport(format, {
        venueName,
        venueLogoUrl,
        report,
        exportedAt: new Date(),
      ...
      console.error("[gratuity-by-waiter-report-export]", err);
      setError(
        err instanceof Error
          ? err.message
          : "Could not export the gratuity by waiter report.",
      );
    } finally {
      setExporting(false);
    }
  ...
  return createPortal(
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="gratuity-by-waiter-report-export-title"
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-black/10 px-5 py-4">
          <div>
            <h2
              id="gratuity-by-waiter-report-export-title"
            ...
            <p className="mt-1 text-sm text-black/55">
              {showMonthSelect
                ? "Choose month and format for daily gratuity by waiter (cash and credit card)."
              ...
              {FORMAT_OPTIONS.map((option) => {
                ...
                return (
                  ...
                  >
                    <input
                      type="radio"
                      name="gratuity-by-waiter-report-export-format"
                      value={option.value}
                      checked={checked}
