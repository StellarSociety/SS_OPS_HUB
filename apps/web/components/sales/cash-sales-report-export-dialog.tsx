...
import {
  ...
  type CashSalesReportMonth,
} from "@/lib/sales/cash-sales-report";
import {
  exportCashSalesReport,
  type CashSalesReportExportFormat,
} from "@/lib/sales/cash-sales-report-export";
...
type CashSalesReportExportDialogProps = {
  open: boolean;
  venueName: string;
...
const FORMAT_OPTIONS: Array<{
  value: CashSalesReportExportFormat;
  label: string;
  description: string;
...
export function CashSalesReportExportDialog({
  open,
  venueName,
  venueLogoUrl,
  userDisplayName,
  ...
  onClose,
}: CashSalesReportExportDialogProps) {
  const availableMonths = useMemo(
    () => listCashSalesReportMonths(records ?? []),
    [records],
  );
  const [month, setMonth] = useState(() =>
    fixedReport?.monthKey ?? defaultCashSalesReportMonth(availableMonths),
  );
  const [format, setFormat] = useState<CashSalesReportExportFormat>("pdf");
  const [exporting, setExporting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const showMonthSelect = allowMonthSelect && Boolean(records);
  ...
  const report = useMemo(() => {
    ...
    return buildCashSalesReportMonth(records ?? [], month);
  ...
  async function handleExport() {
    ...
    setError(null);
    try {
      await exportCashSalesReport(format, {
        venueName,
        venueLogoUrl,
        report,
        exportedAt: new Date(),
      ...
      console.error("[cash-sales-report-export]", err);
      setError(
        err instanceof Error
          ? err.message
          : "Could not export the cash sales report.",
      );
    } finally {
      setExporting(false);
    }
  ...
  return createPortal(
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="cash-sales-report-export-title"
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-black/10 px-5 py-4">
          <div>
            <h2
              id="cash-sales-report-export-title"
            ...
            <p className="mt-1 text-sm text-black/55">
              {showMonthSelect
                ? "Choose month and format for daily cash sales."
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
              ...
              {FORMAT_OPTIONS.map((option) => {
                ...
                return (
                  ...
                  >
                    <input
                      type="radio"
                      name="cash-sales-report-export-format"
                      value={option.value}
                      checked={checked}
          ...
          <dl className="space-y-2 rounded-lg border border-black/10 bg-black/[0.02] p-4 text-sm">
            ...
            <div className="flex justify-between gap-3 border-t border-black/10 pt-2">
              <dt className="font-medium text-[#3D421F]">Cash sales total</dt>
