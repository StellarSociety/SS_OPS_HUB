...
import {
  listAssetTermsEmailSends,
  previewAssetTermsEmail,
  sendAssetTermsEmail,
  type AssetTermsEmailPreview,
  type AssetTermsEmailSendRecord,
} from "@/lib/actions/hr-asset-terms-email";
...
export function AssetTermsEmailSendButton({
  staffId,
  fullName,
  empNo,
  sentCount = 0,
  ...
  const [previewMeta, setPreviewMeta] =
    useState<AssetTermsEmailPreview | null>(null);
  ...
  const [sendHistory, setSendHistory] = useState<
    AssetTermsEmailSendRecord[] | null
  >(null);
  ...
  function loadSendHistory() {
    ...
    startHistoryTransition(async () => {
      const result = await listAssetTermsEmailSends({ staffId });
      if (!result.ok) {
        setSendHistory([]);
  ...
  function openPreview() {
    ...
    startTransition(async () => {
      const result = await previewAssetTermsEmail({ staffId });
      if (!result.ok) {
  ...
  async function handleSend() {
    ...
    try {
      const result = await sendAssetTermsEmail({
        staffId,
        draft: {
          to,
          subject,
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Could not send asset T&Cs email.",
      );
      setSendPhase("error");
    }
  }
  ...
  return (
    <>
      <button
        ...
        title={
          disabled
            ? "Assign assets before sending T&Cs"
            : visibleSentCount > 0
              ? `${visibleSentCount} T&Cs email${visibleSentCount === 1 ? "" : "s"} sent — email ${fullName} again`
              : sentTo
                ? `Sent to ${sentTo}`
                : `Email ${fullName} assets on hand & T&Cs`
        }
        aria-label={`Send asset T&Cs email to ${fullName}${
          visibleSentCount > 0
            ...
            >
              {dialogStep === "confirm" ? (
                  ...
                  <div className="mt-4 rounded-lg border border-black/10 bg-black/[0.02] px-3 py-3 text-sm text-[#3D421F]">
                    ...
                    <div className="mt-3 border-t border-black/8 pt-3">
                      ...
                      {historyPending && sendHistory === null ? (
                        ...
                        <ul className="mt-2 max-h-40 space-y-2 overflow-y-auto">
                          {sendHistory.map((send) => {
                            ...
                            if (send.itemCount != null) {
                              metaParts.push(
                                `${send.itemCount} item${send.itemCount === 1 ? "" : "s"}`,
                              );
                            }
                        ...
                        <p className="mt-1.5 text-xs text-black/45">
                          No asset T&amp;Cs email has been sent to this
                          employee yet.
                        </p>
                      )}
                    </div>
                ...
                <div
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="preview-asset-terms-email-title"
                ...
                >
                  <div className="border-b border-black/8 px-6 py-4">
                    <h2
                      id="preview-asset-terms-email-title"
                      className="font-serif text-xl text-[#3D421F]"
                    >
                      {dialogTitle}
                    </h2>
