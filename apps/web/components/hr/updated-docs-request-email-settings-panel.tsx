...
import { saveUpdatedDocsRequestEmailSettings } from "@/lib/actions/hr-updated-docs-request-email";
import {
  UPDATED_DOCS_REQUEST_EMAIL_TEMPLATE_CODES,
...
export function UpdatedDocsRequestEmailSettingsPanel({
  settings,
  connectionFromEmail = "",
}: {
  ...
  async function handleSave(formData: FormData) {
    ...
    setStatusError(null);
    const result = await saveUpdatedDocsRequestEmailSettings(formData);
    if (!result.ok) {
      setStatusError(result.error);
      return result;
    }
    setStatusMessage("Updated docs request email settings saved.");
    return result;
  }
      ...
      >
        <label className="flex items-start gap-2 rounded-lg border border-black/10 bg-[var(--venue-secondary,#F0F3DD)]/35 px-3 py-2.5 text-sm text-[#3D421F]">
          ...
          <span>
            <span className="block font-medium">
              Enable updated docs request emails
            </span>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="udr_recipient_field">Send to employee email</Label>
            <select
              id="udr_recipient_field"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="udr_from_email">From email (optional)</Label>
            <Input
              id="udr_from_email"
              name="from_email"
              type="email"
        ...
        <div className="space-y-1.5">
          <Label htmlFor="udr_subject">Subject</Label>
          <Input
            id="udr_subject"
            name="subject"
            value={subject}
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <Label htmlFor="udr_message">Message</Label>
            <button
              type="button"
          ...
          {messageHelpOpen ? (
            <div className="space-y-3 rounded-lg border border-black/10 bg-white/70 p-3">
              <p className="text-xs text-black/55">
                ...
                is filled with that employee&apos;s currently missing fields
                when the email is sent.
              </p>
              <ul className="grid gap-2 sm:grid-cols-2">
                {UPDATED_DOCS_REQUEST_EMAIL_TEMPLATE_CODES.map((item) => (
                  <li key={item.code}>
                    <button
                      type="button"
                      className={cn(
