...
import {
  ...
  exportPoolCollections,
  sumPoolCollectionsMonths,
  type PoolCollectionsExportFormat,
  type PoolCollectionsExportMonth,
} from "@/lib/hr/benefits/pool-collections-export";
import { cn } from "@/lib/utils";

type BenefitPoolCollectionsExportDialogProps = {
  open: boolean;
...
const FORMAT_OPTIONS: Array<{
  value: PoolCollectionsExportFormat;
  label: string;
  description: string;
...
function defaultSelectedKeys(
  ...
  const key = defaultPoolCollectionsMonthKey(months, initialMonthKey);
  return key ? [key] : [];
}

export function BenefitPoolCollectionsExportDialog({
  open,
  months,
  venueName,
  venueLogoUrl,
  ...
  initialMonthKey,
  onClose,
}: BenefitPoolCollectionsExportDialogProps) {
  ...
  const [format, setFormat] = useState<PoolCollectionsExportFormat>("pdf");
  ...
  const summary = useMemo(
    () => sumPoolCollectionsMonths(selectedMonths),
    [selectedMonths],
  );
  ...
  async function handleExport() {
    ...
    setError(null);
    try {
      await exportPoolCollections(format, {
        venueName,
        venueLogoUrl,
      ...
      console.error("[pool-collections-export]", err);
      setError(
        err instanceof Error
          ? err.message
          : "Could not export pool collections.",
      );
    } finally {
      setExporting(false);
    }
  ...
  return createPortal(
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="pool-collections-export-title"
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-black/10 px-5 py-4">
          <div>
            <h2
              id="pool-collections-export-title"
              className="font-serif text-xl text-[#3D421F]"
            >
              Export pool collections
            </h2>
            <p className="mt-1 text-sm text-black/55">
              Choose months and format.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
              ...
              {FORMAT_OPTIONS.map((option) => {
                ...
                return (
                  ...
                  >
                    <input
                      type="radio"
                      name="pool-collections-export-format"
                      value={option.value}
                      checked={checked}
          ...
          <dl className="space-y-2 rounded-lg border border-black/10 bg-black/[0.02] p-4 text-sm">
            ...
            <div className="flex justify-between gap-3">
              <dt className="text-black/55">OS&amp;E ({osePercent}%)</dt>
              <dd className="font-medium tabular-nums text-[#3D421F]">
                {formatAed(summary.ose)}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              ...
              <dd className="font-medium tabular-nums text-[#3D421F]">
                {formatAed(summary.activities)}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-black/55">Rounding</dt>
              <dd className="font-medium tabular-nums text-[#3D421F]">
                {formatAed(summary.rounding)}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-black/55">Withheld retain</dt>
              <dd className="font-medium tabular-nums text-[#3D421F]">
                {formatAed(summary.withheldRetain)}
              </dd>
            </div>
            <div className="flex justify-between gap-3">
              <dt className="text-black/55">Deducted</dt>
              <dd className="font-medium tabular-nums text-[#3D421F]">
                {formatAed(summary.deducted)}
              </dd>
            </div>
            <div className="flex justify-between gap-3 border-t border-black/10 pt-2">
              <dt className="font-medium text-[#3D421F]">Total</dt>
