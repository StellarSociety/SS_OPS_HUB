...
import { saveUniformReplacementEmailSettings } from "@/lib/actions/hr-uniform-replacement-email";
import {
  UNIFORM_REPLACEMENT_EMAIL_TEMPLATE_CODES,
...
export function UniformReplacementEmailSettingsPanel({
  settings,
  connectionFromEmail = "",
}: {
  ...
  async function handleSave(formData: FormData) {
    ...
    setStatusError(null);
    const result = await saveUniformReplacementEmailSettings(formData);
    if (!result.ok) {
      setStatusError(result.error);
      return result;
    }
    setStatusMessage("Uniform replacement email settings saved.");
    return result;
  }
    ...
    <Card className="space-y-6 p-5">
      <div>
        <h2 className="font-serif text-lg text-[#3D421F]">
          Uniform replacement deduction email
        </h2>
        <p className="mt-1 text-sm text-black/55">
          Notifies the employee that a replacement cost will be deducted from
          the next payroll. Sent after an employee-paid replacement from Uniform
          → Employees. Delivery uses Venue Settings → Email config.
        </p>
      </div>
      ...
      >
        <label className="flex items-start gap-2 rounded-lg border border-black/10 bg-[var(--venue-secondary,#F0F3DD)]/35 px-3 py-2.5 text-sm text-[#3D421F]">
          ...
          <span>
            <span className="block font-medium">
              Enable uniform replacement deduction emails
            </span>
            <span className="mt-0.5 block text-xs text-black/55">
              When off, the send button after a charged replacement stays
              unavailable.
            </span>
          </span>
        </label>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1.5">
            <Label htmlFor="ure_recipient_field">Send to employee email</Label>
            <select
              id="ure_recipient_field"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="ure_from_email">From email (optional)</Label>
            <Input
              id="ure_from_email"
              name="from_email"
              type="email"
        ...
        <div className="space-y-1.5">
          <Label htmlFor="ure_subject">Subject</Label>
          <Input
            id="ure_subject"
            name="subject"
            value={subject}
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <Label htmlFor="ure_message">Message</Label>
            <button
              type="button"
          ...
          {messageHelpOpen ? (
            <div className="space-y-3 rounded-lg border border-black/10 bg-white/70 p-3">
              <p className="text-xs text-black/55">
                ...
                is filled with the amount queued for the next payroll.
              </p>
              <ul className="grid gap-2 sm:grid-cols-2">
                {UNIFORM_REPLACEMENT_EMAIL_TEMPLATE_CODES.map((item) => (
                  <li key={item.code}>
                    <button
                      type="button"
                      className={cn(
