...
import {
  previewAssetReplacementEmail,
  sendAssetReplacementEmail,
  type AssetReplacementEmailPreview,
} from "@/lib/actions/hr-asset-replacement-email";
...
export function AssetReplacementEmailSendButton({
  staffId,
  fullName,
  empNo,
  replacementIds,
  ...
  const [previewMeta, setPreviewMeta] =
    useState<AssetReplacementEmailPreview | null>(null);
  ...
  function openPreview() {
    ...
    startTransition(async () => {
      const result = await previewAssetReplacementEmail({
        staffId,
        replacementIds,
        deductionAmount,
      });
  ...
  async function handleSend() {
    ...
    try {
      const result = await sendAssetReplacementEmail({
        staffId,
        replacementIds,
        deductionAmount,
        draft: {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Could not send asset replacement email.",
      );
      setSendPhase("error");
    }
  }
  ...
  return (
    <>
      <Button
        ...
        aria-label={`Send asset replacement deduction email to ${fullName}`}
        className={cn(
            ...
            >
              {dialogStep === "confirm" ? (
                ...
                <div
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="preview-asset-replacement-email-title"
                ...
                >
                  <div className="border-b border-black/8 px-6 py-4">
                    <h2
                      id="preview-asset-replacement-email-title"
                      className="font-serif text-xl text-[#3D421F]"
                    >
                      {dialogTitle}
                    </h2>
