...
import { saveAssetReplacementEmailSettings } from "@/lib/actions/hr-asset-replacement-email";
import {
  ASSET_REPLACEMENT_EMAIL_TEMPLATE_CODES,
...
export function AssetReplacementEmailSettingsPanel({
  settings,
  connectionFromEmail = "",
}: {
  ...
  async function handleSave(formData: FormData) {
    ...
    setStatusError(null);
    const result = await saveAssetReplacementEmailSettings(formData);
    if (!result.ok) {
      setStatusError(result.error);
      return result;
    }
    setStatusMessage("Asset replacement email settings saved.");
    return result;
  }
    ...
    <Card className="space-y-6 p-5">
      <div>
        <h2 className="font-serif text-lg text-[#3D421F]">
          Asset replacement deduction email
        </h2>
        <p className="mt-1 text-sm text-black/55">
          Notifies the employee that a replacement cost will be deducted from
          the next payroll. Sent after an employee-paid replacement from Assets
        ...
        <label className="flex items-start gap-2 rounded-lg border border-black/10 bg-[var(--venue-secondary,#F0F3DD)]/35 px-3 py-2.5 text-sm text-[#3D421F]">
          ...
          <span>
            <span className="block font-medium">
              Enable asset replacement deduction emails
            </span>
          ...
          {messageHelpOpen ? (
            <div className="space-y-3 rounded-lg border border-black/10 bg-white/70 p-3">
              ...
              <ul className="grid gap-2 sm:grid-cols-2">
                {ASSET_REPLACEMENT_EMAIL_TEMPLATE_CODES.map((item) => (
                  <li key={item.code}>
                    <button
                      type="button"
                      className={cn(
