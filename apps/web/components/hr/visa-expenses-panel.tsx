...
import { VisaExpensesExportDialog } from "@/components/hr/visa-expenses-export-dialog";
...
import {
  ...
  type VisaExpenseLine,
  type VisaExpenseMonth,
} from "@/lib/hr/types";
import { cn } from "@/lib/utils";

type VisaExpensesPanelProps = {
  months: VisaExpenseMonth[];
  employeeRows: VisaEmployeeRow[];
  canManage?: boolean;
  venueName: string;
...
function visaExpenseTypeTagClass(name: string): string {
  if (name === VISA_EXPENSE_CATEGORY.penalties) {
    return "border-red-200 bg-red-100 text-red-800";
  }
  if (name === VISA_EXPENSE_CATEGORY.cancelations) {
    return "border-amber-200 bg-amber-100 text-amber-900";
  }
  if (name === VISA_EXPENSE_CATEGORY.processing) {
    return "border-emerald-200 bg-emerald-100 text-emerald-800";
  }
  return "border-black/10 bg-black/[0.04] text-[#3D421F]";
}

function VisaExpenseTypeTag({ name }: { name: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium",
        visaExpenseTypeTagClass(name),
      )}
    >
      {name}
    </span>
...
export function VisaExpensesPanel({
  months,
  employeeRows,
  canManage = false,
  venueName,
  ...
  const employeesById = useMemo(() => {
    const map = new Map<string, VisaEmployeeRow>();
  ...
  function openEmployeeVisa(staffId: string) {
    if (!canManage) return;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-black/50">
            Monthly costs by type — Visa Processing, Visa Penalties, and Visa
            Cancelations.
          </p>
        </div>
        {years.length > 0 ? (
      ...
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[#d8d9c8] bg-white/40 px-6 py-16">
          <p className="text-center text-sm text-muted-foreground">
            No visa expenses found
            {yearFilter !== "all" ? ` for ${yearFilter}` : ""}. Issue dates on
            employee visa records drive this report.
          </p>
        </div>
      ) : (
                    ...
                    {month.lines.map((line, lineIndex) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2.5">
                          <VisaExpenseTypeTag name={line.name} />
                        </td>
        ...
        ? createPortal(
            ...
            >
              <div
                role="dialog"
                aria-modal="true"
                aria-labelledby="visa-expense-staff-title"
              ...
              >
                <div className="flex items-start justify-between gap-3 border-b border-black/10 px-5 py-4">
                  <div className="min-w-0">
                    <h2
                      id="visa-expense-staff-title"
                      className="font-nav text-base font-semibold text-[#3D421F]"
                    >
                      <VisaExpenseTypeTag name={staffPopup.line.name} />
                    </h2>
                    <p className="mt-1 text-sm text-black/50">
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          onClick={() => {
                            if (canOpen) openEmployeeVisa(person.staffId);
                          }}
                          onKeyDown={(e) => {
                            if (!canOpen) return;
                            if (e.key === "Enter" || e.key === " ") {
                              e.preventDefault();
                              openEmployeeVisa(person.staffId);
                            }
                          }}
                          className={
                            canOpen
