...
import {
  previewUniformReplacementEmail,
  sendUniformReplacementEmail,
  type UniformReplacementEmailPreview,
} from "@/lib/actions/hr-uniform-replacement-email";
...
export function UniformReplacementEmailSendButton({
  staffId,
  fullName,
  empNo,
  replacementIds,
  ...
  disabled = false,
  className,
  label = "Send deduction notice email",
}: {
  staffId: string;
  fullName: string;
  empNo?: string;
  ...
  const [previewMeta, setPreviewMeta] =
    useState<UniformReplacementEmailPreview | null>(null);
  ...
  function openPreview() {
    ...
    startTransition(async () => {
      const result = await previewUniformReplacementEmail({
        staffId,
        replacementIds,
        deductionAmount,
      });
  ...
  async function handleSend() {
    ...
    try {
      const result = await sendUniformReplacementEmail({
        staffId,
        replacementIds,
        deductionAmount,
        draft: {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Could not send uniform replacement email.",
      );
      setSendPhase("error");
    }
  }
  ...
  return (
    <>
      <Button
        ...
        title={
          disabled
            ? "No deduction amount to notify"
            : sentTo
              ? `Sent to ${sentTo}`
              : `Email ${fullName} about payroll deduction`
        }
        aria-label={`Send uniform replacement deduction email to ${fullName}`}
        className={cn(
            ...
            >
              {dialogStep === "confirm" ? (
                ...
                >
                  ...
                  <div className="mt-4 rounded-lg border border-black/10 bg-black/[0.02] px-3 py-3 text-sm text-[#3D421F]">
                    ...
                    <p className="mt-1 text-xs text-black/50">
                      Deduction {formatAed(deductionAmount)} from next payroll
                    </p>
                  </div>
                ...
                <div
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="preview-uniform-replacement-email-title"
                ...
                >
                  <div className="border-b border-black/8 px-6 py-4">
                    <h2
                      id="preview-uniform-replacement-email-title"
                      className="font-serif text-xl text-[#3D421F]"
                    >
                      {dialogTitle}
                    </h2>
                  ...
                  <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
                    {showSendStatus ? (
                        ...
                        {sendPhase === "sending" ? (
                          <div className="space-y-3">
                            ...
                            <ul className="space-y-1.5 text-xs text-black/50">
                              {SEND_STEPS.map((stepLabel, index) => {
                      ...
                      <>
                        <div className="space-y-1.5">
                          ...
                          <div className="rounded-md border border-black/10 bg-black/[0.02] px-3 py-2.5 text-sm text-[#3D421F]">
                            ...
                            <p className="mt-0.5 text-xs text-black/45">
                              Deduction {previewMeta.deductionAmountLabel}
                            </p>
                          </div>
                        </div>

                        <div className="space-y-1.5">
                          <Label htmlFor="ure-email-to">To</Label>
                          <Input
                            id="ure-email-to"
                            type="email"
                            value={draft.to}
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="ure-email-subject">Subject</Label>
                          <Input
                            id="ure-email-subject"
                            value={draft.subject}
                            onChange={(e) =>
                              setDraft((prev) =>
                                prev
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="ure-email-body">Message</Label>
                          <EmailMessageEditor
                            id="ure-email-body"
                            value={draft.body}
                            onChange={(body) =>
