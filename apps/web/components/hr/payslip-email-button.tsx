...
import { Check, FileText, Mail, XCircle } from "lucide-react";
...
} from "@/lib/actions/hr-payslip-email";
import { cn } from "@/lib/utils";

const SEND_STEPS = [
  "Preparing payslip PDF…",
  "Connecting to mail…",
  "Delivering email…",
  "Confirming delivery…",
] as const;
...
export function PayslipEmailButton({
  ...
  const [previewMeta, setPreviewMeta] = useState<PayslipEmailPreview | null>(
    null,
  );
  ...
  function openPreview() {
    ...
    startTransition(async () => {
      const result = await previewPayslipEmail(payslipId);
      if (!result.ok) {
  ...
  async function handleSend() {
    ...
    try {
      const result = await sendPayslipsEmail([payslipId], {
        draft: {
          to,
          subject,
          body: draft.body,
      ...
      if (result.errors.length > 0) {
        setError(result.errors[0] ?? "Could not send payslip email.");
        setSendPhase("error");
        return;
      }
      if (result.skipped > 0) {
        setError("Skipped — no recipient email on file.");
        setSendPhase("error");
        return;
      }
      setError("Could not send payslip email.");
      setSendPhase("error");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Could not send payslip email.");
      setSendPhase("error");
    }
  }
            ...
            >
              {dialogStep === "confirm" ? (
                ...
                <div
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="preview-payslip-email-title"
                ...
                >
                  <div className="border-b border-black/8 px-6 py-4">
                    <h2
                      id="preview-payslip-email-title"
                      className="font-serif text-xl text-[#3D421F]"
                    >
                      {dialogTitle}
                    </h2>
                    {!showSendStatus ? (
                      <p className="mt-1 text-sm text-black/55">
                        Edit the fields below, then confirm to send the payslip
                        PDF.
                      </p>
                    ) : null}
                  </div>
                  ...
                  <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
                    {showSendStatus ? (
                      ...
                      <>
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="payslip-email-to">To</Label>
                          <Input
                            id="payslip-email-to"
                            type="email"
                            value={draft.to}
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="payslip-email-subject">Subject</Label>
                          <Input
                            id="payslip-email-subject"
                            value={draft.subject}
                            onChange={(e) =>
                              setDraft((prev) =>
                                prev
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="payslip-email-body">Message</Label>
                          <textarea
                            id="payslip-email-body"
                            value={draft.body}
                            onChange={(e) =>
                              setDraft((prev) =>
                                prev
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="rounded-md border border-black/10 bg-black/[0.02] px-3 py-2.5 text-sm text-[#3D421F]">
                            {previewMeta.attachmentFilename ? (
                              <span className="inline-flex items-start gap-2">
                                <FileText className="mt-0.5 size-4 shrink-0 text-[var(--venue-primary,#818a40)]" />
                              ...
                              <span className="text-black/45">
                                No PDF attachment (disabled in email settings)
                              </span>
                            )}
                          </div>
                        </div>
