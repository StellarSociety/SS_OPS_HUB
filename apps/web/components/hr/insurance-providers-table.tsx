...
import { InsuranceProviderDialog } from "@/components/hr/insurance-provider-dialog";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { toast } from "@/components/ui/toast";
import {
  reorderInsuranceProvidersAction,
  setInsuranceProviderArchived,
...
type InsuranceProvidersTableProps = {
...
export function InsuranceProvidersTable({
  ...
  categoryPositionHints = [],
  canManage = false,
}: InsuranceProvidersTableProps) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [search, setSearch] = useState("");
  const [showArchived, setShowArchived] = useState(false);
  const [editProvider, setEditProvider] = useState<InsuranceProvider | null>(
    null,
  );
  ...
  function reorder(draggedId: string, targetId: string) {
    ...
    startReorder(async () => {
      const result = await reorderInsuranceProvidersAction(
  ...
  async function handleArchiveToggle(provider: InsuranceProvider) {
    const archived = Boolean(provider.archived_at);
    const confirmMsg = archived
      ? `Restore "${provider.name}" to the active providers list?`
      : `Archive "${provider.name}"? It will be hidden from employee suggestions.`;
    if (!window.confirm(confirmMsg)) return;

    setArchiveBusyId(provider.id);
    try {
      const result = await setInsuranceProviderArchived({
        id: provider.id,
        archived: !archived,
      });
      if (!result.ok) {
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {filtered.map((provider) => {
                  ...
                  const activeCats = provider.categories.filter(
                    (c) => !c.archived_at,
                  );
                  return (
                    <tr
                      ...
                      <td className="px-4 py-3 align-top">
                        {activeCats.length === 0 ? (
                          ...
                          <ul className="space-y-1">
                            {activeCats.map((cat) => (
                              <li key={cat.id} className="text-[#3D421F]">
                                <span className="font-medium">{cat.name}</span>
                                <span className="text-black/40">
                                  {" "}
                                  · {formatAed(cat.default_medical_value)}
                                </span>
                                {cat.position_defaults.length > 0 ? (
                                  <span className="text-black/35">
                                    {" "}
                                    · {cat.position_defaults.length} position
                                    {cat.position_defaults.length === 1
                                      ? ""
                                      : "s"}
                                  </span>
                                ) : null}
