...
function requestTypeLabel(requestType: string | null): string | null {
  ...
  if (key === "cancel") return "Cancelation";
  return requestType;
}

export function VisaRequestSentEmailsDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const [sends, setSends] = useState<
    VisaRequestEmailSendRecord[] | null
  >(null);
  ...
  useEffect(() => {
    ...
    startTransition(async () => {
      const result = await listVisaRequestEmailSends();
      if (!result.ok) {
        setSends([]);
  ...
  return createPortal(
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="visa-sent-emails-title"
      ...
      >
        <div className="flex items-start justify-between gap-3 border-b border-black/10 px-5 py-4">
          <div>
            <h2
              id="visa-sent-emails-title"
              className="font-nav text-base font-semibold text-[#3D421F]"
            >
              Sent visa requests
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Provider emails sent from Visa → Employees.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          {pending && sends === null ? (
            ...
            <div className="rounded-xl border border-dashed border-black/15 px-4 py-12 text-center text-sm text-black/45">
              No visa request emails have been sent yet.
            </div>
          ) : (
