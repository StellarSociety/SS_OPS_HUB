...
import {
  previewUpdatedDocsRequestEmail,
  sendUpdatedDocsRequestEmail,
  type UpdatedDocsRequestEmailPreview,
} from "@/lib/actions/hr-updated-docs-request-email";
...
export function UpdatedDocsRequestSendButton({
  staffId,
  fullName,
  empNo,
  expiry,
  ...
  const [previewMeta, setPreviewMeta] =
    useState<UpdatedDocsRequestEmailPreview | null>(null);
  ...
  function openPreview() {
    ...
    startTransition(async () => {
      const result = await previewUpdatedDocsRequestEmail({
        staffId,
        expiry: expiry ?? null,
      });
      if (!result.ok) {
  ...
  async function handleSend() {
    ...
    try {
      const result = await sendUpdatedDocsRequestEmail({
        staffId,
        expiry: expiry ?? null,
        draft: {
          to,
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Could not send updated docs request email.",
      );
      setSendPhase("error");
    }
  }
    ...
    <>
      <button
        ...
        title={
          sentTo
            ? `Sent to ${sentTo}`
            : `Request updated documents / missing details from ${fullName}`
        }
        aria-label={`Send updated docs request email to ${fullName}`}
        className={cn(
            ...
            >
              {dialogStep === "confirm" ? (
                <div
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="send-updated-docs-title"
                  className="w-full max-w-md rounded-xl border border-black/10 bg-white p-6 shadow-xl"
                  onMouseDown={(e) => e.stopPropagation()}
                >
                  <h2
                    id="send-updated-docs-title"
                    className="font-serif text-xl text-[#3D421F]"
                  >
                    {dialogTitle}
                  </h2>
                  <p className="mt-1 text-sm text-black/55">
                    Continue to review the request listing this employee&apos;s
                    currently missing details before sending.
                  </p>
                ...
                <div
                  role="dialog"
                  aria-modal="true"
                  aria-labelledby="preview-updated-docs-email-title"
                ...
                >
                  <div className="border-b border-black/8 px-6 py-4">
                    <h2
                      id="preview-updated-docs-email-title"
                      className="font-serif text-xl text-[#3D421F]"
                    >
                      {dialogTitle}
                    </h2>
                  ...
                  <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
                    {showSendStatus ? (
                          ...
                          <div className="flex flex-col items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-5 text-center">
                            ...
                            <div>
                              <p className="font-medium text-emerald-950">
                                Updated docs request delivered
                              </p>
                      ...
                      <>
                        <div className="space-y-1.5">
                          ...
                          <div className="rounded-md border border-black/10 bg-black/[0.02] px-3 py-2.5 text-sm text-[#3D421F]">
                            ...
                            <p className="mt-0.5 text-xs text-black/45">
                              {previewMeta.missingCount} item
                              {previewMeta.missingCount === 1 ? "" : "s"}{" "}
                              requested
                              {previewMeta.docLabel
                                ? ` · ${previewMeta.docLabel}`
                                : ""}
                              {previewMeta.daysStatus
                                ? ` · ${previewMeta.daysStatus}`
                                : ""}
                            </p>
                          </div>
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="udr-email-to">To</Label>
                          <Input
                            id="udr-email-to"
                            type="email"
                            value={draft.to}
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="udr-email-subject">Subject</Label>
                          <Input
                            id="udr-email-subject"
                            value={draft.subject}
                            onChange={(e) =>
                              setDraft((prev) =>
                                prev
                        ...
                        <div className="space-y-1.5">
                          <Label htmlFor="udr-email-body">Message</Label>
                          <EmailMessageEditor
                            id="udr-email-body"
                            value={draft.body}
                            onChange={(body) =>
