...
import { saveUniformTermsEmailSettings } from "@/lib/actions/hr-uniform-terms-email";
import {
  UNIFORM_TERMS_EMAIL_TEMPLATE_CODES,
...
export function UniformTermsEmailSettingsPanel({
  settings,
  connectionFromEmail = "",
}: {
  ...
  async function handleSave(formData: FormData) {
    ...
    setStatusError(null);
    const result = await saveUniformTermsEmailSettings(formData);
    if (!result.ok) {
      setStatusError(result.error);
      return result;
    }
    setStatusMessage("Uniform T&Cs email settings saved.");
    return result;
  }
    ...
    <Card className="space-y-6 p-5">
      <div>
        ...
        <p className="mt-1 text-sm text-black/55">
          Confirms the employee&apos;s current uniforms on hand and includes
          terms for company uniform usage (including salary deduction for loss
          or damage). Sent from Uniform → Employees via T&amp;C&apos;s @.
        ...
        <label className="flex items-start gap-2 rounded-lg border border-black/10 bg-[var(--venue-secondary,#F0F3DD)]/35 px-3 py-2.5 text-sm text-[#3D421F]">
          ...
          <span>
            <span className="block font-medium">
              Enable uniform T&amp;Cs emails
            </span>
            <span className="mt-0.5 block text-xs text-black/55">
              When off, the T&amp;C&apos;s @ control on Uniform Employees stays
              unavailable.
            </span>
          </span>
        </label>
          ...
          {messageHelpOpen ? (
            <div className="space-y-3 rounded-lg border border-black/10 bg-white/70 p-3">
              <p className="text-xs text-black/55">
                ...
                is filled with that employee&apos;s current uniform pieces when
                the email is sent.
              </p>
              <ul className="grid gap-2 sm:grid-cols-2">
                {UNIFORM_TERMS_EMAIL_TEMPLATE_CODES.map((item) => (
                  <li key={item.code}>
                    <button
                      type="button"
                      className={cn(
