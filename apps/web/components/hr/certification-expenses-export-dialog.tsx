...
import {
  defaultCertificationExpenseMonthKey,
  exportCertificationExpenses,
  type CertificationExpensesExportFormat,
} from "@/lib/hr/certification-expenses-export";
...
type CertificationExpensesExportDialogProps = {
  open: boolean;
  months: CertificationExpenseMonth[];
  venueName: string;
  venueLogoUrl?: string | null;
  userDisplayName: string;
  /** Prefill month key (`YYYY-MM`) when opening. */
  initialMonthKey?: string;
  onClose: () => void;
};

const FORMAT_OPTIONS: Array<{
  value: CertificationExpensesExportFormat;
  label: string;
  description: string;
...
export function CertificationExpensesExportDialog({
  open,
  months,
  venueName,
  venueLogoUrl,
  ...
  initialMonthKey,
  onClose,
}: CertificationExpensesExportDialogProps) {
  ...
  const [monthKey, setMonthKey] = useState(() =>
    initialMonthKey && months.some((m) => m.monthKey === initialMonthKey)
      ? initialMonthKey
      : defaultCertificationExpenseMonthKey(months),
  );
  const [format, setFormat] =
    useState<CertificationExpensesExportFormat>("pdf");
  ...
  async function handleExport() {
    if (!report) {
      setError("Select a month with certification expenses.");
      return;
    }
    setExporting(true);
    setError(null);
    try {
      await exportCertificationExpenses(format, {
        venueName,
        venueLogoUrl,
        month: report,
        exportedAt: new Date(),
      ...
      console.error("[certification-expenses-export]", err);
      setError(
        err instanceof Error
          ? err.message
          : "Could not export certification expenses.",
      );
    } finally {
      setExporting(false);
    }
  ...
  return createPortal(
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="cert-expenses-export-title"
      ...
      >
        <div className="flex shrink-0 items-start justify-between gap-3 border-b border-black/10 px-5 py-4">
          <div>
            <h2
              id="cert-expenses-export-title"
              className="font-serif text-xl text-[#3D421F]"
            >
              Export certification expenses
            </h2>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
              ...
              {FORMAT_OPTIONS.map((option) => {
                ...
                return (
                  ...
                  >
                    <input
                      type="radio"
                      name="cert-expenses-export-format"
                      value={option.value}
                      checked={checked}
          ...
          <dl className="space-y-2 rounded-lg border border-black/10 bg-black/[0.02] p-4 text-sm">
            <div className="flex justify-between gap-3">
              <dt className="text-black/55">Certificates</dt>
