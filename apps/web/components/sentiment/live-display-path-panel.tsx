...
export type LiveDisplaySimPageId = "live";

const PAGES: Array<{
  id: LiveDisplaySimPageId;
  label: string;
  icon: typeof Star;
}> = [{ id: "live", label: "Live display", icon: Star }];

export function LiveDisplayPathPanel({
  code,
  selectedId,
  onSelect,
}: {
  code: string;
  selectedId: LiveDisplaySimPageId;
  onSelect: (id: LiveDisplaySimPageId) => void;
}) {
  const href = liveDisplayPath(code);
  ...
  return (
    <Card className={DEVICE_PREVIEW_PATH_PANEL_CLASS}>
      ...
      <nav aria-label="Live display path" className="mt-4 flex flex-col">
