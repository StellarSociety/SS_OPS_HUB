...
type AccountingWelcomeProps = {
  venue: Venue;
  userName?: string | null;
};

export function AccountingWelcome({ venue, userName }: AccountingWelcomeProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 pb-2 pt-6 text-center md:pt-10">
      ...
      <h1 className="font-serif text-3xl font-semibold tracking-tight text-[#3D421F] md:text-4xl">
        {firstName ? `Welcome back, ${firstName}` : "Welcome to Accounting"}
      </h1>
      ...
      <p className="max-w-xl text-sm text-black/55 md:text-base">
        Your accounting hub for {venue.name}. Enter supplier invoices under
        Invoices, or configure entities and the chart of accounts in Settings.
      </p>
    </div>
  );
}
