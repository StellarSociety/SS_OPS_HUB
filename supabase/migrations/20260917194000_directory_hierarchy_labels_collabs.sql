-- Labels above people on the reporting tree, and dashed side-collab links.

ALTER TABLE public.directory_hierarchy_nodes
  ADD COLUMN IF NOT EXISTS label TEXT,
  ADD COLUMN IF NOT EXISTS collabs JSONB NOT NULL DEFAULT '[]'::jsonb;

ALTER TABLE public.directory_hierarchy_nodes
  DROP CONSTRAINT IF EXISTS directory_hierarchy_nodes_label_length;

ALTER TABLE public.directory_hierarchy_nodes
  ADD CONSTRAINT directory_hierarchy_nodes_label_length
  CHECK (label IS NULL OR char_length(btrim(label)) BETWEEN 1 AND 48);
